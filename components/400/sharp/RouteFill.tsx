import * as React from "react";
import type { SVGProps } from "react";
const SvgRouteFill = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width={48}
    height={48}
    viewBox="0 -960 960 960"
    {...props}
  >
    <path d="M200-120v-503.558Q165-637 142.5-665.263 120-693.527 120-729.667 120-776 152.5-808t78-32q45.5 0 77.5 32.138 32 32.139 32 78.051Q340-694 317.5-665.5 295-637 260-624v444h190v-660h310v503.558q35 13.442 57.5 41.807Q840-266.27 840-230q0 45-32.083 77.5Q775.833-120 730-120q-45 0-77.5-32.5T620-230q0-36.297 22.5-65.148Q665-324 700-336v-444H510v660z" />
  </svg>
);
export default SvgRouteFill;
