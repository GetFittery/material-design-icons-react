import * as React from "react";
import type { SVGProps } from "react";
const SvgCircles = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width={48}
    height={48}
    viewBox="0 -960 960 960"
    {...props}
  >
    <path d="M690-390q28-1 55-6t53-13q-15 140-117.5 234.5T438-80q-74 0-139.5-28t-114-76.5Q136-233 108-298.5T80-438q0-142 95-245.5T412-798q-12 26-16.5 54t-5.5 57q-89 21-147 89.5T185-438q0 106 73.5 179.5T438-185q91 0 161.5-57.5T690-390m-5-530q97.5 0 165.75 68.353t68.25 166Q919-588 850.647-519.5q-68.353 68.5-166 68.5T518.5-519.542Q450-588.083 450-686q0-97.5 68.542-165.75Q587.083-920 685-920m-.5 364q51.5 0 90.5-39t39-90.5q0-51.5-39-90.5t-90.5-39q-51.5 0-90.5 39t-39 90.5q0 51.5 39 90.5t90.5 39M439-437" />
  </svg>
);
export default SvgCircles;
