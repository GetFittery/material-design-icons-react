import * as React from "react";
import type { SVGProps } from "react";
const SvgCakeFill = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width={48}
    height={48}
    viewBox="0 -960 960 960"
    {...props}
  >
    <path d="M108.521-74.5v-321.87h742.958V-74.5zm87-381.87v-187.26h250.414v-59.935q-20-14.355-30.5-31.433-10.5-17.077-10.5-41.369 0-15.307 5.619-29.524 5.62-14.218 16.337-24.935L480-886.696l53.109 55.87q10.717 10.717 16.837 24.935 6.119 14.217 6.119 29.524 0 24.292-11 41.369-11 17.078-31 31.433v59.935h250.414v187.26z" />
  </svg>
);
export default SvgCakeFill;
