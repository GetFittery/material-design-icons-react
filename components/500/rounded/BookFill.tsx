import * as React from "react";
import type { SVGProps } from "react";
const SvgBookFill = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width={48}
    height={48}
    viewBox="0 -960 960 960"
    {...props}
  >
    <path d="M222.152-74.022q-27.599 0-47.865-20.265-20.265-20.266-20.265-47.865v-675.696q0-27.697 20.265-48.033 20.266-20.337 47.865-20.337h515.696q27.697 0 48.033 20.337 20.337 20.336 20.337 48.033v675.696q0 27.599-20.337 47.865-20.336 20.265-48.033 20.265zm264.326-743.826v237.413q0 9.196 8.132 14.294 8.132 5.098 16.977.38l54.739-31.891q7.887-4.478 16.737-4.478 8.85 0 17.567 4.478l53.979 31.891q8.771 4.718 17.32-.38t8.549-14.294v-237.413z" />
  </svg>
);
export default SvgBookFill;
